...
if (logoutButton) {
    logoutButton.addEventListener("click", async () => {
        try {
            ...
            if (res.ok) {
                window.location.href = "/"; // URL correcta
            } else {
    ...
    document.querySelector(".add-btn")?.addEventListener("click", async function () {
      ...
      const educador_id = document.getElementById("educador-select").value;
      ...
      if (nombreNino && edad && nombreAcudiente && correoAcudiente) {
        try {
          const response = await fetch("/api/agregar-nino", {
            ...
            body: JSON.stringify({ nombreNino, edad, nombreAcudiente, correoAcudiente, educador_id })
          });
